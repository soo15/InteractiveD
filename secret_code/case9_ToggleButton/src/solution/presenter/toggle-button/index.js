...
export class ToggleButton {
    constructor(configuration) {
        ...
        // 초기 템플릿 display
        this.buttonElements = this.initialize(document.querySelector(configuration.selector), configuration.data);
        ...
        this.buttonElements.forEach((element, index) => {
            element.addEventListener('click', () => {
                ...
                if (this.selectedIndex > -1) {
                    this.offTargetButton(this.buttonElements[this.selectedIndex]);
                }

                this.selectedIndex = index;

                this.onTargetButton(this.buttonElements[this.selectedIndex]);
                this.callback(index);
            })
        })
    }

    onTargetButton(element) {
        element.classList.add('select');
    }

    offTargetButton(element) {
